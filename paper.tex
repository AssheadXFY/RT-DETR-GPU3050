...
yet their feature pyramid network (FPN) necks depend exclusively on spatial-domain convolutions,
overlooking frequency-domain cues that capture fine-grained textures and global structural patterns.
...
yields a \textbf{+1.45} mAP improvement on COCO~2017 (46.73\% $\to$ 48.18\%)
without altering the backbone or decoder.
The 72-epoch FreqSpatial model reaches the official 120-epoch RT-DETRv2-S performance,
demonstrating accelerated convergence through frequency-aware inductive bias.
A +3.4 mAP advantage at epoch~0 further confirms the intrinsic benefit of spectral processing.
FreqSpatial is a plug-and-play module compatible with any FPN-style neck; our implementation is open-source.
\end{abstract}
...
Object detection has advanced markedly with the emergence of DETR-based
...
depends exclusively on spatial-domain convolutions for multi-scale feature
fusion. It overlooks frequency-domain information that encodes
...
results on small-object benchmarks (VisDrone, TinyPerson). FD-RTDETR~\cite{wang2025fdrtdetr}
...
combining Scharr-based edge extraction with FFT convolution remains underexplored
on large-scale general-purpose detection.

We propose \textbf{FreqSpatial}, a plug-and-play frequency-spatial
...
in FSDETR, we redesign the module as a CSP-style drop-in block compatible
with any FPN neck and conduct a large-scale validation
...
vs.\ 14.4\% for the baseline (+3.4), confirming the benefit of frequency-aware
inductive bias.

Our contributions are:
\begin{enumerate}
\item We validate frequency-spatial feature fusion at scale on COCO (80 classes,
  118K images), demonstrating that FreqSpatial generalizes beyond small-object
  datasets to general-purpose detection (+1.45~mAP).
\item We redesign the frequency-spatial module as a plug-and-play CSP block that
  integrates into any FPN-style neck without backbone or decoder changes.
...
Our work validates frequency-spatial fusion (Scharr + FFT convolution) as a
plug-and-play CSP neck block at scale---80 classes, 118K images on COCO~2017,
achieving +1.45 mAP, double the gain of prior frequency-enhanced
RT-DETR methods.
...
Baseline (46.73\%).
Gains are consistent across scales: +1.6~AP$_{50}$, +1.5~AP$_{75}$,
+1.6~AP$_M$, +1.5~AP$_L$, and +1.0~AP$_S$.
At 72 epochs, FreqSpatial reaches the official 120-epoch RT-DETRv2-S performance
(48.1\%~\cite{lv2024rtdetrv2}), demonstrating accelerated convergence.
...
vs.\ 14.4\% for the Baseline (\textbf{+3.4 mAP}). The gain is largest on
...
of 48.18\% at epoch~71. This trajectory shows faster convergence driven by
frequency-aware features. Further gains under the current learning rate schedule
appear limited.
...
However, 38.5~FPS remains well above the real-time threshold (>30~FPS), and the
+1.45~mAP gain offers a favorable accuracy-efficiency trade-off.
...
17.8\% FreqSpatial) and reduces FPS to 48.2. This confirms that
frequency-domain processing, rather than generic attention, drives the
improvement.
...
Fig.~\ref{fig:visual} shows detection comparisons.
FreqSpatial consistently detects objects missed by the Baseline, especially
in cluttered scenes and partially occluded cases.
The frequency branch's global receptive field identifies objects with
ambiguous spatial boundaries, while the Scharr branch improves edge
localization. Gains (Baseline~$\to$~FreqSpatial) are highlighted in green.
...
performance confirm the effectiveness of frequency-aware inductive bias
for object detection.

The main limitation is the increased computational cost from FFT operations.
Future work should pursue more efficient frequency-domain operators,
adaptive frequency band selection, and extension beyond object detection
