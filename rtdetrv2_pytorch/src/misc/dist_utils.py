...
# ---- accelerate backend detection ----

def _get_accelerator():
    """return (device_type_str, torch_submodule) e.g. ('npu', torch.npu), ('cuda', torch.cuda), or ('cpu', None)"""
    try:
        import torch_npu
...
_accel_type, _accel_module = _get_accelerator()
...
def setup_distributed(print_rank: int=0, print_method: str='builtin', seed: int=None, ):
    ...
    try:
        ...
        if is_device_available():
            _accel_module.set_device(rank)
            _accel_module.empty_cache()
        enabled_dist = True
...
def setup_seed(seed: int, deterministic=False):
    ...
    if is_device_available():
        _accel_module.manual_seed_all(seed)
